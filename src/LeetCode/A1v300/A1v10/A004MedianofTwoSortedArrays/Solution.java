package LeetCode.A1v300.A1v10.A004MedianofTwoSortedArrays;
/*
...
class Solution {
    ...
    double find_kth(int[] a, int a_begin, int[] b, int b_begin, int k) {
        ...
        if (mid_a < mid_b) {
            System.out.println("a < b 测试第 " + (k - k / 2) + " 个数 a数组起始位置 " + (a_begin + k / 2) + " , b数组起始位置 " + b_begin);
            return find_kth(a, a_begin + k / 2, b, b_begin, k - k / 2);
        }
        //否则相反
        System.out.println("a > b 测试第 " + (k - k / 2) + " 个数 a数组起始位置 " + a_begin + " , b数组起始位置 " + (b_begin + k / 2));
        return find_kth(a, a_begin, b, b_begin + k / 2, k - k / 2);
    }

    public static void main(String[] args) {
        int[] a = {1, 3, 4, 9, 11, 16, 20};
        int[] b = {2, 5, 4, 23, 25, 59};
